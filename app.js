...
function cariocaSetupHTML() {
  const c = setup.config, plan = c.cariocaRounds;
  return `<div class="field"><label for="carioca-variant">Rondas de Carioca</label><select id="carioca-variant" name="carioca-variant">
    ${!plan ? '<option value="legacy" selected>Sin objetivos · conservar reglas anteriores</option>' : ''}
    <option value="ten" ${plan && c.cariocaVariant === 'ten' ? 'selected' : ''}>10 rondas · incluye Escala sucia y Escala real</option>
    <option value="seven" ${c.cariocaVariant === 'seven' ? 'selected' : ''}>7 rondas · partida corta</option>
    <option value="custom" ${c.cariocaVariant === 'custom' ? 'selected' : ''}>Secuencia personalizada</option>
  </select><small>La variante de 10 rondas es la elegida para esta mesa. Puedes adaptar las combinaciones antes de empezar.</small></div>
  ${plan ? `<p class="note">Gana el menor puntaje al completar las ${plan.length} rondas. Los puntajes se ingresan manualmente.</p><details class="carioca-setup" ${c.cariocaVariant === 'custom' ? 'open' : ''}><summary>Ver y personalizar las ${plan.length} rondas</summary><p class="hint">Edita los nombres y las descripciones o cambia el orden. Los cambios crean una secuencia personalizada.</p><div class="carioca-objectives">${plan.map((r,i) => `<section class="carioca-objective"><div class="section-top"><h3>Ronda ${i+1}</h3><div class="toolbar"><button class="icon-btn" type="button" data-action="move-objective" data-index="${i}" data-direction="-1" aria-label="Subir ronda ${i+1}" ${i===0?'disabled':''}>↑</button><button class="icon-btn" type="button" data-action="move-objective" data-index="${i}" data-direction="1" aria-label="Bajar ronda ${i+1}" ${i===plan.length-1?'disabled':''}>↓</button><button class="icon-btn danger" type="button" data-action="remove-objective" data-id="${r.id}" aria-label="Eliminar ronda ${i+1}">${icon('close')}</button></div></div><div class="field"><label for="objective-name-${r.id}">Combinación requerida</label><input id="objective-name-${r.id}" name="objective-name-${r.id}" value="${esc(r.name)}" maxlength="80" required></div><div class="field"><label for="objective-description-${r.id}">Cómo completarla</label><textarea id="objective-description-${r.id}" name="objective-description-${r.id}" rows="3" maxlength="400" required>${esc(r.description)}</textarea></div></section>`).join('')}</div>${button('add-objective',`${icon('plus')} Agregar ronda`,'secondary wide')}</details>` : '<p class="note">Esta configuración no tiene objetivos. Puedes continuar así o elegir una secuencia para la nueva partida.</p>'}`;
}
function cariocaPlanHTML(g) {
  if (g.mode !== 'cumulative') return '';
  const progress = E.cariocaProgress(g);
  if (!progress) return '<p class="note carioca-notice">Partida anterior sin objetivos definidos. Conserva sus reglas y puntajes originales.</p>';
  const done = new Set(g.rounds.map(r => r.objectiveId));
  return `<details class="panel carioca-plan"><summary>Rondas de esta partida · ${progress.completed}/${progress.total} guardadas</summary><ol>${g.config.cariocaRounds.map((r,i) => `<li ${progress.objective?.id === r.id ? 'class="current" aria-current="step"' : ''}><div><strong>${i+1}. ${esc(r.name)}</strong><span class="badge ${progress.objective?.id === r.id ? 'green' : ''}">${done.has(r.id) ? 'Guardada' : progress.objective?.id === r.id ? 'Actual' : 'Pendiente'}</span></div><p>${esc(r.description)}</p></li>`).join('')}</ol></details>`;
}
function roundDescription(g, round = null) {
  const objective = round ? g.config.cariocaRounds?.find(r => r.id === round.objectiveId) : E.cariocaProgress(g)?.objective;
  return objective ? `<p class="note round-objective">${esc(objective.description)}</p>` : '';
}
function setupHTML() {
  const s = setup, c = s.config, m = E.MODES[s.mode];
  const custom = s.mode === 'custom';
  const rounds = custom ? c.kind === 'rounds' : m.kind === 'rounds';
  const guided = s.mode === 'cumulative' && Array.isArray(c.cariocaRounds);
...
function readSetup() {
  ...
  if (setup.mode === 'cumulative' && c.cariocaRounds) {
    c.cariocaRounds.forEach(r => {
      const name = f.get(`objective-name-${r.id}`), description = f.get(`objective-description-${r.id}`);
      if (name !== null && description !== null) {
        if (name.trim() !== r.name || description.trim() !== r.description) c.cariocaVariant = 'custom';
        r.name = name.trim(); r.description = description.trim();
      }
    });
    c.roundLimit = c.cariocaRounds.length; c.target = 0; c.order = 'low';
  }
...
function atEnd(g) { const progress = E.cariocaProgress(g); return progress ? !progress.objective : E.reached(g) || (g.config.roundLimit > 0 && g.rounds.length >= g.config.roundLimit); }
function gameHTML() {
...
function helpHTML() {
  ...
  <details><summary>Carioca, dominó y juegos de tu mesa</summary><p>Carioca ofrece la secuencia de 10 rondas elegida para esta mesa: dos tríos; un trío y una escala; dos escalas; tres tríos; dos tríos y una escala; un trío y dos escalas; tres escalas; cuatro tríos; Escala sucia; Escala real. La partida corta usa las primeras siete. Puedes personalizar nombres, descripciones y orden antes de empezar. Gana el menor puntaje; la última ronda muestra el resultado automáticamente.</p><p>En esta variante, Escala sucia permite mezclar pintas del as al rey; Escala real exige una misma pinta. Son descripciones editables, no reglas universales. La lista de rondas marca cuál está pendiente y los objetivos se conservan al corregir puntos. Las partidas antiguas sin objetivos continúan con sus reglas originales.</p><p> Dominó permite anotar por persona o equipo y ajustar la meta. Ambos son marcadores: los valores de las cartas, fichas y cierres se acuerdan en la mesa según su variante.</p><p>En «Tu propio juego» puedes elegir rondas o hasta 16 categorías. Cada categoría multiplica la cantidad por su valor, incluso valores negativos para penalizaciones. Guarda las reglas para reutilizarlas.</p></details>
...
function editRound(index) {
  ...
  openDialog(`Editar ${esc(E.roundLabel(g,r))}`, `${roundDescription(g,r)}<p class="hint" style="margin-bottom:16px">El total de todas las rondas se recalculará.</p><div class="stack">${g.players.map(p=>numberField(p.id,esc(p.name),r.scores[p.id],'',g.mode==='uno'?0:-999999)).join('')}</div>`, `${button('delete-round','Eliminar ronda','ghost danger',`data-index="${index}"`)}<button class="btn" type="submit">Guardar cambios</button>`, f=>{
...
function finishGame() {
  const g=game();
  const progress=E.cariocaProgress(g);
  if(progress?.objective)throw Error(`Quedan ${progress.total-progress.completed} rondas por guardar. El resultado aparecerá al completar la secuencia.`);
...
function exportCSV() {
  ...
  if(E.kind(g)==='rounds') { rows.push([],['Ronda',...(E.hasCariocaPlan(g)?['Objetivo','Descripción']:[]),...g.players.map(p=>p.name)]);g.rounds.forEach((r,i)=>{const plan=g.config.cariocaRounds;const index=plan?.findIndex(o=>o.id===r.objectiveId);rows.push([E.hasCariocaPlan(g)?index+1:i+1,...(E.hasCariocaPlan(g)?[plan[index].name,plan[index].description]:[]),...g.players.map(p=>r.scores[p.id])]);}); }
...
document.addEventListener('click', event=>{
  ...
  try {
    if(action==='nav')go(el.dataset.view);
      ...
      readSetup(); const c=setup.config;
      if(c.cariocaRounds.length>=50)throw Error('Puedes usar hasta 50 rondas.');
      c.cariocaRounds.push({id:uid(),name:'Nueva combinación',description:'Describe qué debe completar cada jugador.'});
      c.cariocaVariant='custom';c.roundLimit=c.cariocaRounds.length;render();
      const field=$(`#objective-name-${c.cariocaRounds.at(-1).id}`);field.focus();field.select();
    }
    else if(action==='remove-objective'){
      readSetup();const c=setup.config;
      if(c.cariocaRounds.length===1)throw Error('Conserva al menos una ronda.');
      c.cariocaRounds=c.cariocaRounds.filter(r=>r.id!==el.dataset.id);c.cariocaVariant='custom';c.roundLimit=c.cariocaRounds.length;render();
    }
    else if(action==='move-objective'){
      readSetup();const c=setup.config,i=Number(el.dataset.index),j=i+Number(el.dataset.direction);
      if(j<0||j>=c.cariocaRounds.length)return;
      [c.cariocaRounds[i],c.cariocaRounds[j]]=[c.cariocaRounds[j],c.cariocaRounds[i]];c.cariocaVariant='custom';render();
      document.querySelector(`[data-action="move-objective"][data-index="${j}"][data-direction="${el.dataset.direction}"]`)?.focus();
    }
    ...
    else if(action==='delete-round'){const i=Number(el.dataset.index);confirmAction(`Eliminar ${esc(E.roundLabel(game(),game().rounds[i]))}`, E.hasCariocaPlan(game()) ? 'Este objetivo volverá a quedar pendiente. Las demás rondas conservarán sus objetivos. Puedes deshacerlo.' : 'Se recalcularán todos los totales. Puedes deshacerlo.','Eliminar ronda',()=>commit(n=>{n.rounds.splice(i,1);},'Ronda eliminada.'));}
...
document.addEventListener('submit', event=>{
  if(event.target.id==='setup-form') {
    ...
    try {
      ...
      const scores=readRound();commit(n=>E.appendRound(n,uid(),scores),'Ronda guardada.');
      if(game().finished && E.hasCariocaPlan(game())){
        const result=$('.finish-banner');result.tabIndex=-1;result.scrollIntoView({block:'start'});result.focus({preventScroll:true});
      }
    }catch(e){$('#round-error').textContent=e.message;}
  }
});
...
document.addEventListener('change',event=>{
  if(event.target.id==='carioca-variant'){
    try {
      const variant=event.target.value;readSetup();const c=setup.config;
      if(variant==='custom'){c.cariocaRounds??=E.cariocaPreset();}
      else if(['ten','seven'].includes(variant))c.cariocaRounds=E.cariocaPreset(variant);
      else return;
      c.cariocaVariant=variant;c.roundLimit=c.cariocaRounds.length;c.target=0;c.order='low';render();
      $('#carioca-variant').focus();
