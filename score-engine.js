...
(function (root) {
  ...
  const CARIOCA_ROUNDS = [
    ['Dos tríos', 'Forma dos grupos de tres cartas del mismo valor.'],
    ['Un trío y una escala', 'Forma un grupo de tres cartas del mismo valor y una secuencia de cuatro cartas consecutivas de una misma pinta.'],
    ['Dos escalas', 'Forma dos secuencias de cuatro cartas consecutivas de una misma pinta cada una.'],
    ['Tres tríos', 'Forma tres grupos de tres cartas del mismo valor.'],
    ['Dos tríos y una escala', 'Forma dos grupos de tres cartas del mismo valor y una secuencia de cuatro cartas consecutivas de una misma pinta.'],
    ['Un trío y dos escalas', 'Forma un grupo de tres cartas del mismo valor y dos secuencias de cuatro cartas consecutivas de una misma pinta cada una.'],
    ['Tres escalas', 'Forma tres secuencias de cuatro cartas consecutivas de una misma pinta cada una.'],
    ['Cuatro tríos', 'Forma cuatro grupos de tres cartas del mismo valor.'],
    ['Escala sucia', 'Forma una secuencia completa del as al rey. Puedes mezclar pintas.'],
    ['Escala real', 'Forma una secuencia completa del as al rey de una misma pinta.']
  ];
  function cariocaPreset(variant = 'ten') {
    if (!['ten', 'seven'].includes(variant)) throw Error('Variante de Carioca desconocida.');
    return CARIOCA_ROUNDS.slice(0, variant === 'seven' ? 7 : 10).map(([name, description], i) => ({ id: `carioca-${i+1}`, name, description }));
  }
  const hasCariocaPlan = g => g.mode === 'cumulative' && Array.isArray(g.config.cariocaRounds);
  function cariocaProgress(g) {
    if (!hasCariocaPlan(g)) return null;
    const completed = new Set(g.rounds.map(r => r.objectiveId));
    const index = g.config.cariocaRounds.findIndex(r => !completed.has(r.id));
    return { completed: completed.size, total: g.config.cariocaRounds.length, index, objective: index < 0 ? null : g.config.cariocaRounds[index] };
  }
  function roundLabel(g, round = null) {
    if (!hasCariocaPlan(g)) return `Ronda ${round ? g.rounds.indexOf(round)+1 : g.rounds.length+1}`;
    const plan = g.config.cariocaRounds;
    const index = round ? plan.findIndex(r => r.id === round.objectiveId) : cariocaProgress(g).index;
    return index < 0 ? `${plan.length} de ${plan.length} rondas completadas` : `Ronda ${index+1} de ${plan.length} · ${plan[index].name}`;
  }
  function appendRound(g, id, scores) {
    if (g.finished) throw Error('La partida ya está finalizada.');
    const progress = cariocaProgress(g);
    if (progress && !progress.objective) throw Error('Ya completaste todos los objetivos de Carioca.');
    g.rounds.push({ id, scores: clone(scores), ...(progress ? { objectiveId: progress.objective.id } : {}) });
    g.draft = {};
    if (progress && cariocaProgress(g).index === -1) g.finished = true;
  }
  ...
  const defaultConfig = mode => ({ order: MODES[mode].order, target: MODES[mode].target, roundLimit: mode === 'cumulative' ? 10 : 0, kind: 'rounds', citiesKnights: false, seafarers: false, harbor: false, other: false, categories: [], ...(mode === 'cumulative' ? { cariocaVariant: 'ten', cariocaRounds: cariocaPreset() } : {}) });
  ...
  function validateGame(input) {
    ...
    // Missing plans identify old games. Never infer objectives from their round count.
    if (c.cariocaRounds !== undefined) {
      if (g.mode !== 'cumulative' || !Array.isArray(c.cariocaRounds) || c.cariocaRounds.length < 1 || c.cariocaRounds.length > 50) throw Error('Carioca necesita entre 1 y 50 objetivos.');
      if (!['ten', 'seven', 'custom'].includes(c.cariocaVariant)) throw Error('Variante de Carioca inválida.');
      const objectiveIds = new Set();
      for (const r of c.cariocaRounds) {
        if (!r || !idOK(r.id) || objectiveIds.has(r.id) || !textOK(r.name, 80) || !textOK(r.description, 400)) throw Error('Revisa los nombres y las descripciones de las rondas de Carioca.');
        objectiveIds.add(r.id);
      }
      if (c.order !== 'low' || c.target !== 0 || c.roundLimit !== c.cariocaRounds.length) throw Error('En Carioca con objetivos gana el menor puntaje al completar la secuencia.');
    }
    ...
    if (hasCariocaPlan(g)) {
      const objectiveIds = new Set(c.cariocaRounds.map(r => r.id));
      if (g.rounds.some(r => !objectiveIds.has(r.objectiveId)) || new Set(g.rounds.map(r => r.objectiveId)).size !== g.rounds.length) throw Error('Los objetivos guardados deben existir y no repetirse.');
    } else if (g.mode === 'cumulative' && g.rounds.some(r => r.objectiveId !== undefined)) throw Error('Falta la secuencia de objetivos de esta partida.');
  ...
  const api = { MODES, CATAN, SEVEN, clone, integer, science, catanParts, sevenParts, defense, parts, total, ranking, reached, kind, defaultConfig, newGame, validateGame, validateCatan, cariocaPreset, hasCariocaPlan, cariocaProgress, roundLabel, appendRound };
